...
#define THREE_CHECK_CLOSURE_FUNCTION(func) _Static_assert(THREE_CHECK_SIZEALIGN(__typeof__(func)*, three_closure_function_t), "Closure function must be compatible")

static THREE_ATTR_INLINE_ALWAYS size_t three_closure_get_size(three_closure_t closure) {
    return sizeof(three_closure_t) + closure.env_size;
}

static THREE_ATTR_INLINE_ALWAYS three_closure_flags_t three_closure_get_flags(three_closure_t closure) {
